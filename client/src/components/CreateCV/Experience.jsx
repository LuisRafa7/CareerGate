...
function Experience({ setNumber, curriculumVitae, setCurriculumVitae }) {
  ...
  const handleRemove = (id) => {
    const newEx = experienceArray.filter((one) => one._id !== id);
        ...
        experienceArraySorted.map((one) => {
          return (
            <>
              ...
              >
                {one.company && <h3>{one.company}</h3>}
                {one.profession && <h3>{one.profession}</h3>}
                {one.city && (
                  <div>
                    <h4>
                      {one.city}{" "}
                      {one.country && (
                        <h4 style={{ display: "inline" }}> - {one.country}</h4>
                      )}
                    </h4>
                  </div>
                )}
